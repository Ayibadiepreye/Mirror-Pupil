...
The mobile app obtains the JWT via Firebase Auth (`firebase_auth` +
`google_sign_in`) and refreshes it automatically. On a `401` response the
client forces a refresh and retries the request once.

### POST `/api/users/register-fcm-token`

Register a device FCM push-notification token. Called automatically on
app startup and on every token refresh.
