...
class BillirichyAutonomousManager:
    ...
    - 3.5 hours: Close 50% if trade in profit
    ...
    async def _check_trade(self, trade: ActiveTrade, now: datetime):
        ...
        1. 5 hours → Close 100%
        2. 3.5 hours → Close 50% (if in profit)
        ...
        # 3.5 HOURS: Close 50% if in profit (conditional)
        elif time_since_entry >= timedelta(hours=3, minutes=30):
            if not trade.auto_partial_applied and await self._is_trade_in_profit(trade):
                await self._action_partial_close(trade, 0.50, "3.5-hour autonomous partial close")
                return
