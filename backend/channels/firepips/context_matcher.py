"""
Firepips - Context Matcher
Implements Section 6.3: 9-Level Context Matching for Management Actions
"""
...
BROADCAST_KEYWORDS = [
    ...
    'close trades', 'exit trades', 'close now', 'exit now'
]


class FirepipsContextMatcher:
    """
    9-Level context matching for Firepips management actions.
    ...
    7. Broadcast keyword
    8. Sole trade
    9. All trades (final fallback for broadcast-type messages)
    """
    ...
    async def match_trades(
        ...
        """
        Match management message to active trades using 9-level algorithm.
        ...
        Args:
            ...
            action: Management action type (for validation)
        ...
        if prices:
            matches = await self._match_by_price(all_trades, prices, direction, action)
            if matches:
                logger.info(
        ...
        # Level 9: All trades (final fallback for broadcast-type messages)
        # Only apply for IMPLIED_CLOSE or other broadcast-style actions
        if action in ['IMPLIED_CLOSE', 'CLOSE_ALL']:
            logger.info(
                f"[Context L9] Matched {len(all_trades)} trade(s) by final fallback (broadcast action)"
            )
            return all_trades
    ...
    async def _match_by_price(
        ...
        Direction validation for MODIFY_SL:
        - BUY: new SL must be below market price
        - SELL: new SL must be above market price
        """
        matches = []
    ...
    def _get_pip_tolerance(self, symbol: str) -> float:
        ...
        if 'JPY' in symbol:
            ...
            return 20.0  # 20 points for Dow Jones
        elif symbol == 'USOIL':
            return 0.10  # 10 pips for oil
        else:
